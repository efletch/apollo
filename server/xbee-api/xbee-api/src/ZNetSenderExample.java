...
//                      XBeeAddress64 addr64 = new XBeeAddress64(0, 0x13, 0xa2, 0, 0x40, 0x0a, 0x3e, 0x02);
                        // my other remote
                        XBeeAddress64 addr64 = new XBeeAddress64(0x00, 0x13, 0xA2, 0x00, 0x40, 0x8D, 0x64, 0x48);
                         
                        // create an array of arbitrary data to send
                        int[] payload = new int[] { 5, 6 };
                        ...
                        while (true) {
                                ...
                                System.out.println("request packet bytes (base 16) " + ByteUtils.toBase16(request.getXBeePacket().getPacket()));
                                
                                long start = System.currentTimeMillis();
                                log.info("sending tx packet: " + request.toString());
                                System.out.println("sending tx packet: " + request.toString());
                                
                                try {
                                		System.out.println("sendSynchronous");
                                        TxStatusResponse response = (TxStatusResponse) xbee.sendSynchronous(request, 10*1000);
                                        System.out.println("sent synchronous response");
                                        ...
                                        if (response.isSuccess()) {
                                        		...
                                        		System.out.println("Packet was successfully delivered");
...
//                                                }                                                       
                                        		...
                                        		System.out.println("Packed delivery failed");
                                                ...
                                                log.error("packet failed due to error: " + response.getStatus());
                                        }
                                     ...
                                     //   log.info("Response in " + (System.currentTimeMillis() - start) + ", Delivery status is " + response.getStatus() + ", 16-bit address is " + ByteUtils.toBase16(response.getRemoteAddress16().getAddress())  + ", discovery status is " + response.getStatus());                                 
